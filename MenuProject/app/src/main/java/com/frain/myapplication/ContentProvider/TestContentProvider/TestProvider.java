...
public class TestProvider extends FragmentActivity implements OnClickListener{
	ListView listView;
	...
	public void insert(){
		for(int i=0; i< 20; i++) {
			...
			//Uri uri1 = Uri.parse("content://com.twentythree.myandroidlearn.database.provider.StudentProvider/student");
			Uri uri = getContentResolver().insert(StudentProvider.STUDENT_URI, values);
			System.out.println("insert rowId=" + ContentUris.parseId(uri));
		}
		query();
	}
	...
	public void query(){
		...
		Cursor cursor = getContentResolver().query(StudentProvider.STUDENT_URI, null, null, null, null);
	...
	public void delete(){
		...
		int count = getContentResolver().delete(StudentProvider.STUDENT_URI, null, null);
	...
	public void update(){
		...
		int count = getContentResolver().update(StudentProvider.STUDENT_URI, values, null, null);
